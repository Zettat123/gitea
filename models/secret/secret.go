...
// getScopedSecretsForJob walks up the caller chain (ParentCallJobID) and applies
...
//
// For top-level jobs (ParentCallJobID == 0) the base secrets are returned as-is.
func getScopedSecretsForJob(ctx context.Context, job *actions_model.ActionRunJob, baseSecrets map[string]string) (map[string]string, error) {
	if job == nil || job.ParentCallJobID == 0 {
		return baseSecrets, nil
	}

	caller, err := actions_model.GetRunJobByRunAndID(ctx, job.RunID, job.ParentCallJobID)
	if err != nil {
		return nil, fmt.Errorf("load caller job %d: %w", job.ParentCallJobID, err)
	}
