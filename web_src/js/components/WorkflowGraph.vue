...
function scopedKey(job: {parentCallJobID: number; jobId: string}): string {
  return `${job.parentCallJobID || 0}:${job.jobId}`;
}
...
function buildDirectNeedsMap(jobs: ActionsJob[]): Map<string, string[]> {
  ...
  for (const job of jobs) {
    ...
    const needKeys = (job.needs || []).map((n) => `${job.parentCallJobID || 0}:${n}`);
...
function computeJobLevels(jobs: ActionsJob[]): Map<string, number> {
  // Scope-aware: each job is keyed by `${parentCallJobID}:${jobId}` so the same JobID
  ...
  function dfs(scoped: string, depth: number = 0): number {
    ...
    for (const need of job.needs) {
      const needScoped = `${job.parentCallJobID || 0}:${need}`;
