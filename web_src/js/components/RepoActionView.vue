...
// so the selected row is always visible in the sidebar.
const forcedExpandedJobIDs = computed(() => {
  ...
  while (cur?.parentCallJobID) {
    expanded.add(cur.parentCallJobID);
    cur = jobsByID.get(cur.parentCallJobID);
  }
  return expanded;
});
      ...
      <div class="action-view-left">
        ...
        <ul class="ui relaxed list flex-items-block tw-p-0">
          <li
            ...
            :class="{'selected': props.jobId === item.job.id, 'is-caller-job': item.job.isReusableCaller}"
          ...
          >
            <button
              ...
              @click="toggleCollapsedJob(item.job.id)"
              :title="isJobCollapsed(item.job.id) ? locale.expand : locale.collapse"
            >
            ...
            <a class="tw-contents silenced" :href="item.job.link" :style="{paddingLeft: `${item.depth * 16}px`}">
...
.job-brief-item.is-caller-job {
  background-image: linear-gradient(to right, var(--color-secondary-alpha-30), transparent 70%);
}

.job-brief-item.is-caller-job:hover,
.job-brief-item.is-caller-job.selected {
