...
func expandReusableWorkflow(ctx context.Context, parentJob *actions_model.ActionRunJob) error {
	...
	return createChildRunFromReusableWorkflow(ctx, parentJob, workflowJob, ref, vars)
}
...
func finalizeChildRun(ctx context.Context, childRunID int64, parentJobStatus actions_model.Status) error {
	if !parentJobStatus.IsDone() {
		return nil
	}

	return db.WithTx(ctx, func(ctx context.Context) error {
		return finalizeChildRunTx(ctx, childRunID, parentJobStatus)
	})
}

func finalizeChildRunTx(ctx context.Context, childRunID int64, parentJobStatus actions_model.Status) error {
	childRunJobs, err := actions_model.GetRunJobsByRunID(ctx, childRunID)
	if err != nil {
		return err
	}

	stopped := timeutil.TimeStampNow()
	for _, childRunJob := range childRunJobs {
		if childRunJob.ChildRunID > 0 {
			if err := finalizeChildRunTx(ctx, childRunJob.ChildRunID, parentJobStatus); err != nil {
				return err
			}
		}

		if childRunJob.Status.IsDone() {
			continue
		}

		if childRunJob.TaskID > 0 {
			if err := actions_model.StopTask(ctx, childRunJob.TaskID, parentJobStatus); err != nil {
				return err
			}
			continue
		}

		childRunJob.Status = parentJobStatus
		childRunJob.Stopped = stopped
		if _, err := actions_model.UpdateRunJob(ctx, childRunJob, builder.Eq{"task_id": 0}, "status", "stopped"); err != nil {
			return err
		}
	}
