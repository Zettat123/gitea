...
func GenerateGiteaContext(ctx context.Context, run *actions_model.ActionRun, attempt *actions_model.ActionRunAttempt, job *actions_model.ActionRunJob) GiteaContext {
	...
	if job != nil {
		...
		if job.ParentCallJobID > 0 {
			...
			// See https://gitea.com/gitea/act_runner/src/commit/35834bf8178b7330b4f17315cdb35fa95d4b4f1e/act/runner/expression.go#L511
...
// Lookup is scoped to the same ParentCallJobID — `needs` references resolve within the
// scope of the same reusable workflow call, not across scopes.
